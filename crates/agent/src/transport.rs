...
pub async fn connect_and_serve(ws_url: &str, credential: &str) -> anyhow::Result<()> {
    ...
    while let Some(message) = stream.next().await {
        match message? {
            Message::Text(text) => {
                ...
                let response = handle(server_msg).await;
...
async fn handle(message: ServerMessage) -> AgentMessage {
    match message {
        ...
        ServerMessage::Command {
            ...
            let outcome = crate::ops::run(operation).await;
