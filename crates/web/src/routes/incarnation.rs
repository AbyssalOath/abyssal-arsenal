...
/// -- Cryptkeeper's `path`/`days_until_expiry` don't identify a specific
/// systemd unit, so there's no field here to pre-fill; the banner is all
/// Phase 6 adds here, and the human decides which listed service to
/// restart.
