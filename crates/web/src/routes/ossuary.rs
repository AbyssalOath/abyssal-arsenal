...
/// and pre-fills the Partition Table device field with `prefill_device`
/// when a suggestion carried one -- not every suggestion into this landing
/// page names a device (Cystoolbox's disk-full suggestion only has a
/// filesystem mount point, which nothing here has a field for), so this is
/// `None` in that case and only the context banner shows.
